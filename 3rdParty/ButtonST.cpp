...
void CButtonST::PreSubclassWindow() 
{
    ...
    ModifyStyle(m_nTypeStyle, BS_OWNERDRAW, SWP_FRAMECHANGED);
...
void CButtonST::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
    ...
    {
        if (IsMenu((HMENU)UIntToPtr(lpMeasureItemStruct->itemID)) && BCMenu::IsMenu((HMENU)UIntToPtr(lpMeasureItemStruct->itemID)))
        {
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
    ...
    if (m_bIsPressed)
    {
        ...
        pDC->DrawEdge(itemRect,EDGE_SUNKEN,BF_RECT);    // Draw a sunken face     
    }
    else // ...else draw non pressed button
    {
        ...
        pDC->DrawEdge(itemRect,EDGE_RAISED,BF_RECT);    // Draw a raised face
    } // else
    ...
    if (sTitle.IsEmpty() == FALSE)
    {
        ...
        CFont * const LOldFont = static_cast<CFont *>(pDC->SelectObject(GetFont()));
        // Center text
...
void CButtonST::DrawTheIcon(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpTitle, BOOL bIsPressed, BOOL bIsDisabled)
{
    ...
    pDC->DrawState(    rImage.TopLeft(),
                    rImage.Size(), 
                    m_csIcons[bIsDisabled? 2 : byIndex].hIcon,
                    (DSS_NORMAL), 
                    (CBrush*)NULL);
} // End of DrawTheIcon
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, LPCRECT pRect)
{
    ...
    CRect LRect(pRect->left,pRect->top,pRect->right,pRect->bottom);
