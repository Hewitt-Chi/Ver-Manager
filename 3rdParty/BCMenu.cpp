...
{
    int m;
    int numSubMenus = PtrToInt(m_AllSubMenus.GetUpperBound());
...
BOOL BCMenu::DestroyMenu()
{
    // Destroy Sub menus:
    int m,n;
    int numAllSubMenus = PtrToInt(m_AllSubMenus.GetUpperBound());
    for (n = numAllSubMenus; n>= 0; n--){
        if (m_AllSubMenus[n]==this->m_hMenu)m_AllSubMenus.RemoveAt(n);
    }
    int numSubMenus = PtrToInt(m_SubMenus.GetUpperBound());
    for (m = numSubMenus; m >= 0; m--){
        numAllSubMenus = PtrToInt(m_AllSubMenus.GetUpperBound());
        for (n = numAllSubMenus; n>= 0; n--){
    ...
    // Destroy menu data
    int numItems = PtrToInt(m_MenuList.GetUpperBound());
...
void BCMenu::MeasureItem( LPMEASUREITEMSTRUCT lpMIS )
{
    ...
    if (state & MF_SEPARATOR){
        ...
        if (Win32s!=g_Shell)
            ...
            size.cx += 3*(int)(size.cx/wcslen(lpstrText));
        }
#endif    
                           ...
                           int nIconNormal)
{
    ...
    if (nFlags & MF_POPUP){
        m_AllSubMenus.Add((HMENU)UintToPtr(nID));
        m_SubMenus.Add((HMENU)UintToPtr(nID));
    }
                           ...
                           int nIconNormal)
{
    ...
    if (nFlags & MF_POPUP){
        ...
        m_AllSubMenus.Add((HMENU)UintToPtr(nID));
        m_SubMenus.InsertAt(nPosition,(HMENU)UintToPtr(nID));
    }
                                      ...
                                      CArray<int,int&>&bclocs)
{
    ...
    for (i=0;i<(int)(GetMenuItemCount());++i){
        ...
        if (psubmenu){
            ...
            numsubs=PtrToInt(bcsubs.GetSize());
            foundflag=TRUE;
            for (j=0;j<numsubs;++j){
    ...
    do{
        ...
        if (dwFlags & MF_POPUP){
            ...
            m_Stack[m_Stack.GetUpperBound()]->AppendODMenuW(szCaption,uFlags,
                (UINT)PtrToUint(pSubMenu->m_hMenu), -1);
            ...
            m_Stack[m_Stack.GetUpperBound()]->AppendODMenuW(szCaption, uFlags,
                dwID, -1);
            if (dwFlags & MF_END)m_StackEnd.SetAt(m_Stack.GetUpperBound(),TRUE);
            j = PtrToInt(m_Stack.GetUpperBound());
    ...
    for (int i=0;i<(int)GetMenuItemCount();++i){
        ...
        if (GetSubMenu(i)){
            ...
            ModifyMenu(i,MF_POPUP|MF_BYPOSITION,
                (UINT)PtrToUint(GetSubMenu(i)->m_hMenu),str);
        }
        else{
...
int BCMenu::GetMenuStart(void)
{
    ...
    int nummenulist=PtrToInt(m_MenuList.GetSize());
...
void BCMenu::RemoveTopLevelOwnerDraw(void)
{
    CString str;
    int i=0,j=0;
    int nummenulist=PtrToInt(m_MenuList.GetSize()),menustart;
...
BOOL BCMenu::GetMenuText(UINT id, CString& string, UINT nFlags/*= MF_BYPOSITION*/)
{
    ...
    if (MF_BYPOSITION&nFlags){
        UINT numMenuItems = PtrToUint(m_MenuList.GetUpperBound());
        if (id<=numMenuItems){
...
void BCMenu::SynchronizeMenu(void)
{
    ...
    for (j=0;j<GetMenuItemCount();++j){
        ...
        if (state&MF_POPUP){
            submenu=(UINT)PtrToUint(GetSubMenu(j)->m_hMenu);
...
BOOL BCMenu::AddBitmapToImageList(CImageList *bmplist,UINT nResourceID)
{
    ...
    // O.S.
    if (m_bDynIcons){
        bmplist->Add((HICON)UIntToPtr(nResourceID));
        bReturn=TRUE;
    }
    else{
    ...
    if (MF_BYPOSITION&nFlags){
        ...
        if (uint&MF_SEPARATOR && !(uint&MF_POPUP)){
            ...
            if (NULL==pSubMenu){
                ...
                for (i=PtrToInt(m_MenuList.GetUpperBound());i>=0;i--){
                    if (m_MenuList[i]->nID==(UINT)PtrToUint(pSubMenu->m_hMenu)){
                        delete m_MenuList.GetAt(i);
                        m_MenuList.RemoveAt(i);
                        break;
                    }
...
BOOL BCMenu::DeleteMenu(UINT uiId,UINT nFlags)
{
    if (MF_BYPOSITION&nFlags){
        ...
        if (uint&MF_SEPARATOR && !(uint&MF_POPUP)){
            ...
            int menulistsize=PtrToInt(m_MenuList.GetSize());    
...
BCMenu* BCMenu::AppendODPopupMenuW(wchar_t *lpstrText)
{
    ...
    pSubMenu->checkmapsshare=TRUE;
    pSubMenu->CreatePopupMenu();
    AppendODMenuW(lpstrText,MF_POPUP,(UINT)PtrToUint(pSubMenu->m_hMenu), -1);
    return pSubMenu;
}
...
BCMenu* BCMenu::GetSubBCMenu(wchar_t* lpszSubMenuName)
{
    ...
    if (mdata){
        HMENU bchmenu=(HMENU)UIntToPtr(mdata->nID);
...
{
    int numcurrent=PtrToInt(m_AllImagesID.GetSize());
    int existsloc = -1;
