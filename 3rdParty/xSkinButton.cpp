...
CxSkinButton::~CxSkinButton()
{
    if (m_hClipRgn) DeleteObject(m_hClipRgn);    // free clip region
...
/////////////////////////////////////////////////////////////////////////////
void CxSkinButton::SetFont(LOGFONT *pLogFont, BOOL bRedraw /*= TRUE*/)
{
    ASSERT(pLogFont);
    if (!pLogFont)
        return;
...
void CxSkinButton::SetFont(LPCTSTR lpszFaceName, 
                           int nPointSize,
                           BOOL bRedraw /*= TRUE*/)
{
    ...
    {
        _tcsncpy_s(lf.lfFaceName, sizeof(lf.lfFaceName), lpszFaceName, sizeof(lf.lfFaceName)/sizeof(TCHAR)-1);
    }
...
int CxSkinButton::SetFontPosition(int nHPos, int nVPos)
{
    m_nFormat = 0;    

    if (nHPos == FONT_CENTER)
        m_nFormat |= DT_CENTER | DT_SINGLELINE;
    else if (nHPos == FONT_LEFT)
        m_nFormat |= DT_LEFT | DT_SINGLELINE;
    else if (nHPos == FONT_RIGHT)
        m_nFormat |= DT_RIGHT | DT_SINGLELINE;

    if (nVPos == FONT_VCENTER)
        m_nFormat |= DT_VCENTER;
    else if (nVPos == FONT_TOP)
        m_nFormat |= DT_TOP;
    else if (nVPos == FONT_BOTTOM)
        m_nFormat |= DT_BOTTOM;
...
void CxSkinButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
    ...
    {    
        ...
        //pDC->DrawText(sCaption,&tr, m_nFormat);
    ...
    {                                        
        ...
        m_nFormat |= DT_VCENTER | DT_SINGLELINE;
        //else
                           ...
                           short drawmode, short border, short margin)
{
    ...
    if (mask>0){
        ...
        if (m_hClipRgn) DeleteObject(m_hClipRgn);
...
void CxSkinButton::OnLButtonDblClk(UINT flags, CPoint point) 
{
    ...
    //SendMessage(WM_LBUTTONDBLCLK, flags, MAKELPARAM(point.x, point.y));
...
void CxSkinButton::OnShowWindow(BOOL bShow, UINT nStatus)
{
    ...
    CWnd *LPrnt = GetParent();
    if (LPrnt) {
    //LPrnt->ScreenToClient(&LRect);
    //LPrnt->InvalidateRect(&LRect);
    LPrnt->RedrawWindow();
    }
    }*/
...
void CxSkinButton::SendNotifyMessageToParent(UINT AMessage, UINT AFlags, CPoint &APoint)
{
    CWnd *LParent = GetParent();

    if (LParent)
    {
        TButtonUpDownNotification    LNtfn = {AFlags, APoint};
