 ...
 *	ChartCtrl.cpp
 *
 *	Written by C嶮ric Moonen (cedric_moonen@hotmail.com)
 *
 *
 *
...
CChartCtrl::CChartCtrl()
{
	...
	m_SelectRecipeBarPoint.x = 0;
	m_SelectRecipeBarPoint.y = 0;

	for(int I =0; I < 10;I++)
	{
		m_ArmRecipeValue[I] = 0;
	}

	m_BarSize                       = 0;

	m_RecipeBarChart                = NULL;

	m_IsEnableUpData                = false;
}
...
void CChartCtrl::SetChartRect(CRect* ARect, int ARectSize)
{
	for(int Idx = 0; Idx < ARectSize; Idx++)
	{
        m_RecipeChartRect[Idx] = ARect[Idx];
	}

}

void CChartCtrl::SetBarChart(CChartBarSerie *ABarChart , int ASize)
{
   if(ABarChart && ASize <= 10)
   {
	   m_RecipeBarChart = ABarChart; 
       m_BarSize = ASize;
	   m_IsEnableUpData = false;
        for( int Idx = 0;Idx < ASize ; Idx++)
		 {
			 const CRect LRect = m_RecipeBarChart->GetBarRectangle(Idx, m_RecipeBarChart->GetMinorOffset());
			 m_RecipeChartRect[Idx] = LRect;

			 m_ArmRecipeValue[Idx] = m_RecipeBarChart->GetYPointValue(Idx);
		 }
...
void CChartCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
     ...
     if(nFlags == MK_LBUTTON)
	 {
		...
		if(m_RecipeBarChart != NULL)
		{
			int LPos = GetRectID(point);
			TRACE("LPos = %d\n",LPos); 
			if(LPos != -1  && m_RecipeChartRect[LPos] != 0 )
			{
				double LYValue = m_RecipeBarChart->GetYPointValue(LPos);     // 取出被選中的Bar Chart 的值
				
				if(point.y < (m_SelectRecipeBarPoint.y - 30) )
				{
					
					LYValue += 0.1;
					if(LYValue >=0.1)
					{
						m_RecipeBarChart->SetYPointValue(LPos , LYValue);
						m_SelectRecipeBarPoint.y = point.y;                  //當Bar Chart獲得新值時，則重新設定被選中的位置
						m_IsEnableUpData = true;
						for(unsigned int Idx = 0;Idx < 10 ; Idx++)           // 更新Bar Chart範圍
						{
							const CRect LRect = m_RecipeBarChart->GetBarRectangle(Idx, m_RecipeBarChart->GetMinorOffset());
							m_RecipeChartRect[Idx] = LRect;	
						}
				...
				else if(point.y > (m_SelectRecipeBarPoint.y + 30))
				{
					LYValue -= 0.1;
					if(LYValue >=0.1)
					{
						m_RecipeBarChart->SetYPointValue(LPos , LYValue);
						m_SelectRecipeBarPoint.y = point.y;
						m_IsEnableUpData = true;
...
bool CChartCtrl::IsUpdataEnable()
{
  return m_IsEnableUpData;
}
int CChartCtrl::GetRectID(CPoint APoint)
{

	for(int Idx = 0; Idx < 10 ; Idx++)
	{
		if(m_RecipeChartRect[Idx].PtInRect(APoint))
			return Idx;
	}
	return -1;
}
void CChartCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(LPos != -1  && m_RecipeChartRect[LPos] != 0)
	{		
		m_SelectRecipeBarPoint = point;
		TRACE("LButton Dwon x= %d , y =%d\n ",m_SelectRecipeBarPoint.x,m_SelectRecipeBarPoint.y);
	}
...
void CChartCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if(m_RecipeBarChart)
	{
		for(int Idx = 0; Idx < m_BarSize ; Idx++)
		{
			m_ArmRecipeValue[Idx] =  m_RecipeBarChart->GetYPointValue(Idx);
		}
	}
	ReleaseCapture();                   // 釋放滑鼠 讓其他視窗可以接收滑鼠輸入
	/*m_bLMouseDown = false;
	if (m_bZoomEnabled)
	{
...
void CChartCtrl::GetBarCharValue(double *AValue)
{
	if(AValue)
	{
		memcpy_s(AValue,sizeof(m_ArmRecipeValue),m_ArmRecipeValue,sizeof(m_ArmRecipeValue));
        m_IsEnableUpData = false;
	}
}
