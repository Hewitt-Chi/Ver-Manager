...
#define IDB_MATRIXLARGE                 102
#define IDB_MATRIXSMALL                 103
#define IDB_MATRIXTINY                  104
...
void CMatrixStatic::RefreshText()
{
    if (m_iMaxYChars == 1)
    {
        if (m_bAutoPad == true)
...
void CMatrixStatic::DoScroll(int speed, BOOL dir)
{
    if (m_btimer == false)
    {
        m_btimer = true;
        m_iDirection  = dir;
        m_iTimer = SetTimer(2, speed, NULL); 
        ASSERT(m_iTimer);
    }
}
...
COLORREF SEGM_COLORS[3] = {
    ...
    HMODULE     LModule = LoadLibrary("MatrixStatic.dll");

    if (LModule)
    {
        bmp = ::CreateMappedBitmap(LModule, m_iImagematrix, 0, ColorMap, 3);
        FreeLibrary(LModule);
    }
    else
    {
