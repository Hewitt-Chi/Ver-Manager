...
void CBarChart::CChartBar::CreateLabelFont()
{
    ...
    strcpy_s(m_lf.lfFaceName, sizeof(m_lf.lfFaceName), "Tahoma");
    m_lf.lfHeight=-10;
    m_lf.lfWidth=0;
    m_lf.lfEscapement = 0;
...
int nWidth=200;     // Modify by Mike, 200 --> 210
int nHeight=100;    // Modify by Mike, 100 --> 170
...
BOOL CChartTip::Create(CWnd *pParentWnd, BOOL *pbEnable, CString* lpszTitle, int nDelay)
{
    ...
    m_rcClient = CRect(0, 0, nWidth , nHeight);
    ...
    */
    ...
    memBitmap.CreateBitmap(nWidth,nHeight,1,GetDeviceCaps(memDC.m_hDC,BITSPIXEL),NULL);
...
void CChartTip::SetText(CString strTemp)
{
    ...
    m_pointOld = point;
    // Draw tip
    memDC.FillRgn( &m_rgnCopy, &m_brBK);
    memDC.FrameRgn( &m_rgnCopy, &m_brFrame, TIP_FRAME_THICKNESS, TIP_FRAME_THICKNESS);
    ...
    if (nIndex!=-1)
    {
        ...
        pOldFont = (CFont*) memDC.SelectObject(&m_fontBold);

        memDC.SetTextColor(TIP_TITLE_COLOR);
        // Draw label
        memDC.DrawText(m_szText, 
    ...
    // Draw rest
    pOldFont = (CFont*) memDC.SelectObject(&m_fontNorm);
    int i = 1;
    memDC.SetTextColor(TIP_TEXT_COLOR);
    while (1)
    {
        ...
        HFONT LFont=CreateFont(22,0,0,0,FW_BOLD,0,0,0,0,0,0,2,0,"SYSTEM_FIXED_FONT"), LTmpFont; // modify by Mike, 22 -->20
        LTmpFont = (HFONT)SelectObject(memDC,LFont);

        CRect LRect = CRect(m_rcClient.left + TIP_LEFT_MARGIN, 
            m_rcClient.top + TIP_TOP_MARGIN + i*TIP_TEXT_MAX_HEIGHT + 2,
            m_rcClient.right - TIP_LEFT_MARGIN,
            m_rcClient.top  + i*TIP_TEXT_MAX_HEIGHT + 50);
        WCHAR LUnicodeChars[256];
        memcpy(LUnicodeChars, "0", 256);
        MultiByteToWideChar(CP_ACP,0,m_szText.GetBuffer(0),-1,LUnicodeChars,256); 
        int J = 0;
        for (J = 0; LUnicodeChars[J] != '\0'; J++);


        TextOutW(memDC, LRect.left, LRect.top, LUnicodeChars, J);
        SelectObject(memDC,LTmpFont);
        DeleteObject(LFont);

        i++;
        memDC.SetTextColor(TIP_PERCENT_COLOR);
    }    

    memDC.SelectObject(pOldFont);
    //    CDC *LpDC = GetDC();
    m_DC->BitBlt(0,0,nWidth,nHeight,&memDC,0,0,SRCCOPY);
...
void CChartTip::OnPaint() 
{
    ...
    while (1)
    {
        ...
        HFONT LFont=CreateFont(20,0,0,0,FW_BOLD,0,0,0,0,0,0,2,0,"SYSTEM_FIXED_FONT"), LTmpFont;
        LTmpFont = (HFONT)SelectObject(memDC,LFont);
        CRect LRect = CRect(m_rcClient.left + TIP_LEFT_MARGIN, 
            m_rcClient.top + TIP_TOP_MARGIN + i*TIP_TEXT_MAX_HEIGHT + 2,
            m_rcClient.right - TIP_LEFT_MARGIN,
...
BOOL CBarChart::CChartDatabase::Prepare(ULONG ulODBCVer)
{
    ...
    m_sqlRet = SQLSetEnvAttr(m_hOdbcEnv, SQL_ATTR_ODBC_VERSION,
        (SQLPOINTER)ULongToPtr(ulODBCVer), SQL_IS_INTEGER);
    if (m_sqlRet != SQL_SUCCESS && m_sqlRet != SQL_SUCCESS_WITH_INFO)
    {
